...
public class ComputeFigure extends Applet
{
	...
	double CenterPoint = x + (width/2.0);
	double CenterPoint2 = y + (height/2.0);
		...
		if(figure.equals("Circle") )
		{
			...
			g.drawString("Top Left Point = (" + nf.format(x)+","+nf.format(y)+")",0,465);
			g.drawString("Center Point = (" + nf.format(CenterPoint)+","+nf.format(CenterPoint2)+")",0,475);			
			g.drawString("Perimeter = "+nf.format(perimeter)+" units squared",0,485);			
			g.drawString("Area = " + nf.format(area)+"units",0,495);
		...
		else
		{
			...
			double perimeter = (height * 2.0) + (width * 2);
